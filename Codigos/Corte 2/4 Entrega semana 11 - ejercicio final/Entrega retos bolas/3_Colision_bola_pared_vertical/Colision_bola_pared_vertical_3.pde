...
CODIGO DE BOLA COLISIONANDO EN UNA PARED VERTICAL
RETO 3/5

INCLUYE:
- REBOTE EN BORDES VENTANA
- REBOTE PARED DERECHA TENIENDO EN CUENTA SU ANGULO DE COLISION
*/
ArrayList<Ball> balls;  // ArrayList para almacenar las bolas
int numBalls = 1;  // Número de bolas - 2 para el ejercicio 2
float bounce = -1.0;  // Factor de rebote - es "la fuerza del rebote"
float wallX;  // Posición x de la pared adicional
float wallWidth = 20; // Ancho de la pared

void setup() {
  size(600, 400);
  balls = new ArrayList<Ball>();  // Inicializar el ArrayList
  wallX = width - wallWidth; // Posición x de la pared adicional
...
void draw() {
  ...
  // Dibujar la pared
  fill(150); // Color gris
  rect(wallX, 0, wallWidth, height); // Dibuja la pared
  ...
  //Genera una "matriz" para relacionar los datos de las bolas creadas
...
class Ball {
  ...
  void checkWalls() {
    if (x + radius > width) {
      x = width - radius; // Mover la bola al borde derecho
      vx *= bounce; // Invertir la velocidad en x para el rebote
    } else if (x - radius < 0) {
      x = radius; // Mover la bola al borde izquierdo
      vx *= bounce; // Invertir la velocidad en x para el rebote
    }
    
    // Verificar la colisión con la pared adicional a la derecha
    if (x + radius > wallX) {
      x = wallX - radius; // Mover la bola al borde de la pared
      vx *= bounce; // Invertir la velocidad en x para el rebote
    }
    
    if (y + radius > height) {
      y = height - radius; // Mover la bola al borde inferior
      vy *= bounce; // Invertir la velocidad en y para el rebote
    } else if (y - radius < 0) {
      y = radius; // Mover la bola al borde superior
      vy *= bounce; // Invertir la velocidad en y para el rebote
    }
  }
